                ...
                .ifPresent(tabs -> {
                    ...
                    RecipeTransferHelpers.getMissingItems(
                        ...
                        for (RecipeInputSlotEmiSlotWidget slot : transferResult.getSlotsMissing()) {
                            ...
                            GuiComponent.fill(matrices, bounds.x(), bounds.y(), bounds.x() + bounds.width(), bounds.y() + bounds.height(), RecipeTransferResult.SLOT_COLOR_MISSING);
                        }
                        for (RecipeInputSlotEmiSlotWidget slot : transferResult.getSlotsCraftable()) {
                            Bounds bounds = slot.getSlotWidget().getBounds();
                            GuiComponent.fill(matrices, bounds.x(), bounds.y(), bounds.x() + bounds.width(), bounds.y() + bounds.height(), RecipeTransferResult.SLOT_COLOR_CRAFTABLE);
                        }

                        // Render overlay on button
                        Bounds bounds = fillButton.get().getBounds();
                        GuiComponent.fill(matrices, bounds.x(), bounds.y(), bounds.x() + bounds.width(), bounds.y() + bounds.height(), transferResult.getButtonHighlightColor());
                    });
                });
    }
}
